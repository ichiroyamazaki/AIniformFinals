...
class ImprovedUniformTracker:
    ...
    def __init__(self, course_type="BSBA_MALE"):
        ...
        self.stable_detection_threshold = 0.5  # Minimum time between updates for stability
...
# Import user's detection system
from vscode.detection_system import (
    REQUIRED_PARTS, MODELS, CONF_THRESHOLDS, 
    get_detected_classes, detect_uniform, show_result_message
)

# Note: DetectionSystem class is defined below in this file

class DetectionSystem:
    def __init__(self, model_path="bsba_male.pt", conf_threshold=0.5, iou_threshold=0.45, cam_index=0):
    ...
    def start_detection(self, person_id, person_name, person_type, ui_callback=None):
        ...
        # Configure camera settings for better visibility
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        self.cap.set(cv2.CAP_PROP_BRIGHTNESS, 0.5)  # Increase brightness
        self.cap.set(cv2.CAP_PROP_CONTRAST, 0.5)    # Increase contrast
        self.cap.set(cv2.CAP_PROP_SATURATION, 0.5)  # Increase saturation
        self.cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25)  # Enable auto exposure
        self.cap.set(cv2.CAP_PROP_EXPOSURE, -6)     # Set exposure
        ...
        print(f"✅ Detection started for {person_name}")
        return True
    ...
    def _detection_loop(self):
        """Main detection loop integrated with UI"""
        try:
            # Determine model and parameters based on person info
            course_key = self._get_course_key()
            model_path = MODELS.get(course_key, "bsba_male.pt")
            conf_threshold = CONF_THRESHOLDS.get(course_key, 0.25)
            required_parts = REQUIRED_PARTS.get(course_key, [])
            
            print(f"🔍 Detecting uniform for {course_key} (model={model_path}, confidence={conf_threshold})")
            ...
            detection_history = defaultdict(lambda: deque(maxlen=8))
            ...
            while self.detection_active and self.cap and self.cap.isOpened():
                ...
                now = time.time()
                for n in names:
                    detection_history[n].append(now)
                
                # Get confirmed parts
                confirmed_parts = {p for p, times in detection_history.items() if len(times) >= 8}
                
                # Use original frame without bounding boxes
                display = frame.copy()
                
                # Add uniform status overlay
                self._draw_uniform_status(display, required_parts, confirmed_parts, start_time)
                ...
                if all(p in confirmed_parts for p in required_parts):
                    ...
                    self._show_complete_message(display)
                    break
                
                # No timeout - run until all parts are detected
    ...
    def _get_course_key(self):
        """Get course key for model selection"""
        if hasattr(self, 'current_person_id') and self.current_person_id:
            # Get student information from Firebase
            student_info = self._get_student_info_by_rfid(self.current_person_id)
            if student_info:
                course = student_info.get('course', 'Unknown')
                gender = student_info.get('gender', 'Unknown')
                
                # Map course to combined key for ICT/BSCPE
                if course.upper() in ["ICT", "BSCPE"] and gender.upper() == "MALE":
                    return "ICT_BSCPE_MALE"
                else:
                    return f"{course.upper()}_{gender.upper()}"
        
        # Default to combined ICT/BSCPE male
        return "ICT_BSCPE_MALE"

    def _get_student_info_by_rfid(self, rfid):
        """Get student info by RFID - connect to main UI's Firebase"""
        # Get the main UI instance to access Firebase
        if hasattr(self, 'main_ui') and self.main_ui:
            return self.main_ui.get_student_info_by_rfid(rfid)
        else:
            # Fallback - return default BSCPE student
            return {
                'course': 'BSCPE',
                'gender': 'Male',
                'name': 'Test Student'
    ...
    def _draw_uniform_status(self, frame, required_parts, confirmed_parts, start_time):
        """Draw uniform status on frame"""
        ...
        cv2.putText(frame, f"Detection time: {int(elapsed_time)}s", (10, 30), font, 0.7, (0, 255, 255), 2)
        cv2.putText(frame, "Press 'q' to stop detection", (10, 55), font, 0.5, (255, 255, 255), 1)
        
        # Check if uniform is complete
        missing_parts = [part for part in required_parts if part not in confirmed_parts]
        is_complete = len(missing_parts) == 0
        
        # Display overall status
        if is_complete:
            cv2.putText(frame, "✅ COMPLETE UNIFORM", (10, 80), font, 0.8, (0, 255, 0), 2)
            cv2.putText(frame, "All required parts detected!", (10, 105), font, 0.6, (0, 255, 0), 2)
        else:
            cv2.putText(frame, "❌ INCOMPLETE UNIFORM", (10, 80), font, 0.8, (0, 0, 255), 2)
            cv2.putText(frame, f"Missing: {len(missing_parts)} parts", (10, 105), font, 0.6, (0, 0, 255), 2)
        
        # Uniform parts status
        y = 130
        for part in required_parts:
            color = (0, 255, 0) if part in confirmed_parts else (0, 0, 255)
            mark = "✔" if part in confirmed_parts else "✘"
            cv2.putText(frame, f"{mark} {part}", (10, y), font, 0.6, color, 2)
            y += 25

    def _show_complete_message(self, frame):
        """Show complete uniform message"""
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(frame, "✅ COMPLETE UNIFORM", (40, 100), font, 1.0, (0, 200, 0), 3, cv2.LINE_AA)
...
class GuardMainControl:
    def __init__(self):
        ...
        self.conf_threshold = 0.2  # Very low threshold to show all detections
        self.iou_threshold = 0.3   # Lower threshold for better overlap detection
                ...
                try:
                    ...
                    for camera_index in [0, 1, 2]:
                        ...
                        if test_cap.isOpened():
                            # Configure test camera settings
                            test_cap.set(cv2.CAP_PROP_BRIGHTNESS, 0.5)
                            test_cap.set(cv2.CAP_PROP_CONTRAST, 0.5)
                            test_cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25)
                            ret, frame = test_cap.read()
                            if ret and frame is not None:
                                print(f"SUCCESS: Camera available on index {camera_index}")
                                camera_found = True
                                test_cap.release()
                                break
                            test_cap.release()
...
# Camera method removed - will be replaced with user's detection file
    ...
    def _camera_display_loop(self):
        """Camera display loop - placeholder for user's detection file"""
        pass
    ...
    def update_camera_feed(self, frame):
        ...
        try:
            if hasattr(self, 'camera_label') and self.camera_label and self.root.winfo_exists():
                # Resize frame to fit camera label (16:9 aspect ratio)
    ...
    def handle_permanent_student_timein(self, rfid, student_info):
        ...
        try:
            ...
            # Start detection for permanent students
            try:
                person_name = student_info['name']
                self.start_person_detection(rfid, person_name, "student")
                self.add_activity_log(f"Detection started for permanent student: {person_name}")
            except Exception as e:
    ...
    def create_camera_feed_section(self, parent):
        ...
        self.camera_label = tk.Label(
            ...
            relief='sunken',
            bd=3,
            width=80,  # Set proper width for video box
            height=20  # Set proper height for video box
        )
    ...
    def start_person_detection(self, person_id, person_name, person_type):
        ...
        try:
            ...
            model_path = self._determine_model_path(person_id, person_type)
            ...
            # Start detection using new integrated DetectionSystem
            ...
            if CV2_AVAILABLE and YOLO_AVAILABLE:
                ...
                print("WARNING: CV2 or YOLO not available, using simulation")
                self.add_activity_log("WARNING: CV2 or YOLO not available, using simulation")
    ...
    def _determine_model_path(self, person_id, person_type):
        """Determine which YOLO model to use based on person ID, course, and gender"""
        if person_type.lower() == 'student':
            # Get student information from Firebase using RFID
            student_info = self.get_student_info_by_rfid(person_id)
            if not student_info:
                print(f"WARNING: Could not get student info for {person_id}, using default model")
                return "bsba_male.pt"
            
            course = student_info.get('course', 'Unknown')
            gender = student_info.get('gender', 'Unknown')
            
            print(f"DEBUG: Student {person_id} - Course: {course}, Gender: {gender}")
            print(f"DEBUG: Course check - '{course.upper()}' in {['BUSINESS', 'BUSINESS AND MANAGEMENT', 'BSBA', 'BUSINESS MANAGEMENT']}")
            
            # Model mapping from the new detection code (ICT and BSCPE combined)
            MODELS = {
                "BSBA_MALE": "bsba_male.pt",
                "BSBA_FEMALE": "bsba_female.pt", 
                "ICT_BSCPE_MALE": "bsba_male.pt",  # Combined ICT and BSCPE
                "ICT_FEMALE": "bsba_female.pt",
                "BSCPE_FEMALE": "bsba_female.pt",
            }
            
            # Create key for model lookup - combine ICT and BSCPE for male students
            if course.upper() in ["ICT", "BSCPE"] and gender.upper() == "MALE":
                key = "ICT_BSCPE_MALE"
            else:
                key = f"{course.upper()}_{gender.upper()}"
            
            # Check for exact match first
            if key in MODELS:
                model = MODELS[key]
                print(f"SUCCESS: Found exact model match: {key} -> {model}")
                self.add_activity_log(f"SUCCESS: Found exact model match: {key} -> {model}")
            else:
                # Fallback matching for partial course names
                if "BUSINESS" in course.upper() or "BSBA" in course.upper():
                    if gender.upper() == 'MALE':
                        model = "bsba_male.pt"
                    elif gender.upper() == 'FEMALE':
                        model = "bsba_female.pt"
                    else:
                        model = "bsba_male.pt"  # Default to male if gender unknown
                elif course.upper() in ["ICT", "BSCPE"]:
                    # ICT and BSCPE use the same model
                    if gender.upper() == 'FEMALE':
                        model = "bsba_female.pt"
                    else:
                        model = "bsba_male.pt"  # Default to male (covers both ICT and BSCPE)
                else:
                    # Default to male ICT model for unknown courses
                    model = "bsba_male.pt"
            
            print(f"DEBUG: Selected model: {model}")
            return model
        else:
            # Default to male ICT model for non-students
            return "bsba_male.pt"
...
once camera is fully initialized"""
    ...
    def start_detection_with_existing_camera(self):
        """Start detection using the existing camera from guard login"""
        try:
            print("INFO: Starting detection with existing camera...")
            self.add_activity_log("INFO: Starting detection with existing camera...")
            print(f"DEBUG: Camera cap exists: {hasattr(self, 'camera_cap')}")
            self.add_activity_log(f"DEBUG: Camera cap exists: {hasattr(self, 'camera_cap')}")
            print(f"DEBUG: Camera cap opened: {self.camera_cap.isOpened() if hasattr(self, 'camera_cap') and self.camera_cap else False}")
            self.add_activity_log(f"DEBUG: Camera cap opened: {self.camera_cap.isOpened() if hasattr(self, 'camera_cap') and self.camera_cap else False}")
            
            # Open camera for student detection
            print("INFO: Opening camera for student detection...")
            self.add_activity_log("INFO: Opening camera for student detection...")
            
            # Test camera availability first
            camera_available, camera_index = self.test_camera_availability()
            if not camera_available:
                print("ERROR: No camera available for detection")
                self.add_activity_log("ERROR: No camera available for detection")
                print("DEBUG: Camera check failed - falling back to simulation")
                self.add_activity_log("DEBUG: Camera check failed - falling back to simulation")
                self.simulate_detection_results()
                return
            
            print(f"SUCCESS: Camera found on index {camera_index}")
            self.add_activity_log(f"SUCCESS: Camera found on index {camera_index}")
            
            # Initialize camera for detection with proper settings
            self.camera_cap = cv2.VideoCapture(camera_index)
            if not self.camera_cap.isOpened():
                print("ERROR: Failed to open camera for detection")
                self.add_activity_log("ERROR: Failed to open camera for detection")
                return False
                
            # Configure camera settings for better visibility
            self.camera_cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
            self.camera_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            self.camera_cap.set(cv2.CAP_PROP_BRIGHTNESS, 0.5)  # Increase brightness
            self.camera_cap.set(cv2.CAP_PROP_CONTRAST, 0.5)    # Increase contrast
            self.camera_cap.set(cv2.CAP_PROP_SATURATION, 0.5)  # Increase saturation
            self.camera_cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25)  # Enable auto exposure
            self.camera_cap.set(cv2.CAP_PROP_EXPOSURE, -6)     # Set exposure
            self.camera_cap.set(cv2.CAP_PROP_FPS, 30)          # Set FPS
            
            # Start camera display loop
            self.camera_active = True
            camera_thread = threading.Thread(target=self._camera_display_loop, daemon=True)
            camera_thread.start()
            print("SUCCESS: Camera display loop started")
            self.add_activity_log("SUCCESS: Camera display loop started")
            
            # Determine model path based on person info
            model_path = self.current_model_path
            print(f"INFO: Using model: {model_path}")
            self.add_activity_log(f"INFO: Using model: {model_path}")
            print(f"DEBUG: Model path: {model_path}")
            self.add_activity_log(f"DEBUG: Model path: {model_path}")
            
            # Check if model file exists (try multiple paths)
            model_found = False
            script_dir = os.path.dirname(os.path.abspath(__file__))
            possible_paths = [
                model_path,
                ...
                os.path.join("C:\\4th yr\\cursor\\Ai2", model_path),
                os.path.join(script_dir, "..", model_path)
            ]
            
            print(f"DEBUG: Script directory: {script_dir}")
            print(f"DEBUG: Current working directory: {os.getcwd()}")
            print(f"DEBUG: Looking for model: {model_path}")
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    model_path = path
                    model_found = True
                    print(f"SUCCESS: Model file found at: {path}")
                    self.add_activity_log(f"SUCCESS: Model file found at: {path}")
                    break
            ...
            if not model_found:
                ...
                for path in possible_paths:
                    print(f"  - {path}")
                self.add_activity_log(f"ERROR: Model file not found: {model_path}")
                print("DEBUG: Model file check failed - falling back to simulation")
                self.add_activity_log("DEBUG: Model file check failed - falling back to simulation")
                self.simulate_detection_results()
                return
            
            print(f"SUCCESS: Model file found: {model_path}")
            self.add_activity_log(f"SUCCESS: Model file found: {model_path}")
            
            # Initialize detection system with existing camera
            print("DEBUG: Initializing DetectionSystem...")
            self.add_activity_log("DEBUG: Initializing DetectionSystem...")
            self.detection_system = DetectionSystem(
                model_path=model_path,
                conf_threshold=self.conf_threshold,
                iou_threshold=self.iou_threshold,
                cam_index=0  # We'll use the existing camera
            )
            
            # Load model
            print("DEBUG: Loading YOLO model...")
            self.add_activity_log("DEBUG: Loading YOLO model...")
            if not self.detection_system.load_model():
                print("ERROR: Failed to load YOLO model")
                self.add_activity_log("ERROR: Failed to load YOLO model")
                print("DEBUG: Model loading failed - falling back to simulation")
                self.add_activity_log("DEBUG: Model loading failed - falling back to simulation")
                self.simulate_detection_results()
                return
            
            print("SUCCESS: YOLO model loaded")
            self.add_activity_log("SUCCESS: YOLO model loaded")
            
            # Use existing camera instead of initializing new one
            print("DEBUG: Assigning existing camera to detection system...")
            self.add_activity_log("DEBUG: Assigning existing camera to detection system...")
            self.detection_system.cap = self.camera_cap
            
            # Set detection session duration (no timeout - runs until complete)
            self.detection_session_duration = None  # No timeout - runs until complete
            self.detection_start_time = time.time()
            
            # Start detection thread
            print("DEBUG: Starting detection thread...")
            self.add_activity_log("DEBUG: Starting detection thread...")
            self.detection_active = True
            self.detection_thread = threading.Thread(target=self._detection_loop_with_existing_camera, daemon=True)
            self.detection_thread.start()
            
            # Initialize improved uniform tracker based on course and gender
            # Get the current person ID from the detection context
            current_person_id = getattr(self, 'current_person_id', None)
            if not current_person_id:
                # Try to get it from the last person entry
                current_person_id = getattr(self, 'last_person_id', None)
            
            student_info = self.get_student_info_by_rfid(current_person_id) if current_person_id else None
            if student_info:
                course = student_info.get('course', 'Unknown').upper()
                gender = student_info.get('gender', 'Unknown').upper()
                course_key = f"{course}_{gender}"
                
                # Initialize improved uniform tracker
                self.improved_uniform_tracker = ImprovedUniformTracker(course_key)
                print(f"DEBUG: Improved uniform tracker initialized for {course_key}")
                self.add_activity_log(f"DEBUG: Improved uniform tracker initialized for {course_key}")
            else:
                # Fallback to BSBA_MALE if no student info
                self.improved_uniform_tracker = ImprovedUniformTracker("BSBA_MALE")
                print("DEBUG: Improved uniform tracker initialized with fallback (BSBA_MALE)")
                self.add_activity_log("DEBUG: Improved uniform tracker initialized with fallback (BSBA_MALE)")
            
            print(f"SUCCESS: Detection started with existing camera - will run until complete")
            self.add_activity_log(f"SUCCESS: Detection started with existing camera - will run until complete")
            
        except Exception as e:
            print(f"ERROR: Failed to start detection with existing camera: {e}")
            self.add_activity_log(f"ERROR: Failed to start detection with existing camera: {e}")
            print("DEBUG: Exception occurred - falling back to simulation")
            self.add_activity_log("DEBUG: Exception occurred - falling back to simulation")
            import traceback
            traceback.print_exc()
            self.simulate_detection_results()
    
    def _detection_loop_with_existing_camera(self):
        """Detection loop using existing camera integrated into main UI - runs until complete"""
        try:
            frame_count = 0
            last_ui_update = 0
            detection_start_time = time.time()
            last_text_update = 0  # Track last text update time
            
            print(f"INFO: Detection session started - will run until complete")
            ...
            while self.detection_active:
                try:
                    ...
                    # Use existing camera
                    if self.detection_system and self.detection_system.cap and self.detection_system.cap.isOpened():
                        ret, frame = self.detection_system.cap.read()
                        if not ret:
                            time.sleep(0.01)
                            continue
                        
                        # Process frame with enhanced detection system (every 2nd frame for better accuracy)
                        if frame_count % 2 == 0:  # Process every 2nd frame for better accuracy
                            if frame_count % 60 == 0:  # Print every 60th frame to reduce spam
                                print(f"DEBUG: Processing frame {frame_count} - elapsed: {elapsed_time:.1f}s")
                            
                            # Get course and gender for proper model selection
                            current_person_id = getattr(self, 'current_person_id', None)
                            if not current_person_id:
                                current_person_id = getattr(self, 'last_person_id', None)
                            
                            student_info = self.get_student_info_by_rfid(current_person_id) if current_person_id else None
                            course = student_info.get('course', 'Unknown').upper() if student_info else 'Unknown'
                            gender = student_info.get('gender', 'Unknown').upper() if student_info else 'Unknown'
                            course_key = f"{course}_{gender}"
                            
                            # Get confidence threshold for this course
                            conf_threshold = CONF_THRESHOLDS.get(course_key, CONF_THRESHOLD_DEFAULT)
                            
                            # Use original frame without bounding boxes
                            processed_frame = frame.copy()
                            
                            # Run detection to get detected classes
                            results = self.detection_system.model(frame, conf=conf_threshold)
                            detected_classes = get_detected_classes(results, conf_threshold, frame.shape)
                            
                            # Update improved uniform tracking
                            uniform_status = ""
                            is_complete = False
                            if hasattr(self, 'improved_uniform_tracker'):
                                is_complete, status_text = self.improved_uniform_tracker.update_detections(detected_classes, current_time)
                                uniform_status = status_text
                                
                                # Log complete uniform detection and stop detection
                                if is_complete:
                                    self.add_activity_log(f"SUCCESS: {status_text} - All uniform components detected!")
                                    print(f"SUCCESS: {status_text}")
                                    # Stop detection when complete
                                    self.stop_detection()
                                    break
                        ...
                        # Draw overlay with uniform status (no time remaining)
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        cv2.putText(processed_frame, f"Elapsed: {int(elapsed_time)}s", (10, 30), font, 0.7, (255, 255, 0), 2)
                        
                        # Get current confirmed parts for stable display
                        if hasattr(self, 'improved_uniform_tracker'):
                            confirmed_parts = self.improved_uniform_tracker.confirmed_parts
                            required_parts = self.improved_uniform_tracker.required_parts
                        else:
                            confirmed_parts = set()
                            required_parts = REQUIRED_PARTS.get(course_key, [])
                        ...
                        for part in required_parts:
                            color = (0, 255, 0) if part in confirmed_parts else (0, 0, 255)
                            mark = "✔" if part in confirmed_parts else "✘"
                            cv2.putText(processed_frame, f"{mark} {part}", (10, y), font, 0.6, color, 2)
                            y += 25
                        
                        # No text overlay - just clean camera feed with bounding boxes
                        # The bounding boxes will show that detection is working
                        
                        # Debug: Print detection details (less frequently)
                        if detections and frame_count % 30 == 0:  # Print every 30th frame
                            for i, det in enumerate(detections):
                                print(f"DEBUG: Detection {i}: {det}")
                        
                        # Convert frame to PhotoImage for Tkinter with proper sizing
                        frame_rgb = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2RGB)
                        
                        # Resize to proper video box format (16:9 aspect ratio)
                        target_width = 640
                        target_height = 360  # 16:9 aspect ratio
                        frame_resized = cv2.resize(frame_rgb, (target_width, target_height))
                        
                        # Debug: Print frame info
                        if frame_count % 60 == 0:  # Print every 60th frame
                            print(f"DEBUG: Frame shape: {frame_resized.shape}, Detections: {len(detections) if detections else 0}")
                        ...
                        # Update camera label with detection feed
                        if hasattr(self, 'camera_label') and self.camera_label and self.root.winfo_exists():
                            # Configure label to show image properly
                            self.camera_label.config(
                                image=photo, 
                                text="",
                                width=80,  # Use character-based width
                                height=20,  # Use character-based height
                                relief='sunken',
                                bd=2
                            )
                            self.camera_label.image = photo  # Keep a reference
                            
                            # Debug: Confirm image update
                            if frame_count % 60 == 0:
                                print(f"DEBUG: Camera label updated with frame {frame_count}")
                        
                        # Debug: Check if frame has bounding boxes (less frequently)
                        if detections and frame_count % 30 == 0:
                            print(f"SUCCESS: Frame processed with {len(detections)} detections")
                    ...
                    if current_time - last_ui_update > 2.0:  # Update every 2 seconds
                        self.update_detection_status_in_ui(detections)
                        last_ui_update = current_time
                    
                    time.sleep(0.1)  # ~10 FPS for more stable detection
                    
                except Exception as e:
                    print(f"ERROR: Error in detection loop: {e}")
                    time.sleep(0.1)
                    
        except Exception as e:
            print(f"ERROR: Detection loop failed: {e}")
        finally:
            print("INFO: Detection session ended")
    
    def update_detection_status_in_ui(self, detections):
        ...
        try:
            ...
            if hasattr(self, 'detection_start_time') and hasattr(self, 'detection_session_duration'):
                elapsed_time = time.time() - self.detection_start_time
                remaining_time = max(0, self.detection_session_duration - elapsed_time)
                time_text = f"⏱️ Time Remaining: {remaining_time:.1f}s"
            else:
                time_text = "⏱️ Detection Active"
            
            status_text = f"📷 LIVE CAMERA FEED - DETECTION ACTIVE\n\n🎥 Camera: ACTIVE\n🔍 Detections: {detection_count}\n🤖 Model: {self.current_model_path}\n{time_text}\n\nDetection running...\n\n💡 Camera shows live feed with bounding boxes"
            ...
            print(f"ERROR: Error updating detection status in UI: {e}")
    
    def start_real_detection(self):
        """Start real detection system with comprehensive error handling"""
        try:
            if not CV2_AVAILABLE or not YOLO_AVAILABLE:
                print("WARNING: YOLO or OpenCV not available - using simulation")
                self.simulate_detection_results()
                return
            
            print("INFO: Starting real detection system...")
            ...
            # Initialize camera
            if not self.detection_system.init_camera():
            ...
            print("SUCCESS: Real detection started successfully")
            
        except Exception as e:
            print(f"ERROR: Failed to start real detection: {e}")
            traceback.print_exc()
            self.simulate_detection_results()
    
    def _detection_loop(self):
        """Optimized detection loop using detected.py approach"""
        try:
            frame_count = 0
            last_ui_update = 0
            ...
            while self.detection_active:
                try:
                    ...
                    if self.detection_system and self.detection_system.cap and self.detection_system.cap.isOpened():
                        ...
                        # Debug: Print frame info
                        print(f"DEBUG: Frame shape: {processed_frame.shape}, Detections: {len(detections) if detections else 0}")
                        
                        # Display the camera feed with detections
                        cv2.imshow('Live Detection Feed', processed_frame)
                        
                        # Handle window close
                        if cv2.waitKey(1) & 0xFF == ord('q'):
                            print("INFO: Detection stopped by user (Q key pressed)")
                            self.stop_detection()
                            break
                        
                        # Debug: Check if frame has bounding boxes
                        if detections:
                            print(f"SUCCESS: Frame processed with {len(detections)} detections")
                        else:
                            print("INFO: Frame processed with no detections")
                        
                    else:
                        # Demo mode - create demo frame
                        if hasattr(self.detection_system, 'create_demo_frame'):
                            frame = self.detection_system.create_demo_frame()
                        else:
                            # Fallback demo frame
                            import numpy as np
                        ...
                        detections = []  # No real detections in demo mode
                    
                    # Update UI more frequently for better responsiveness
                    if current_time - last_ui_update > 0.05:  # 20 FPS for UI updates
                        # Debug: Check if we're updating the UI
                        if detections:
                            print(f"🔍 Updating UI with {len(detections)} detections")
                        self.root.after(0, self._update_camera_preview, processed_frame)
                        last_ui_update = current_time
                    
                    # Process detection results with improved accuracy
                    if detections:
                        print(f"🔍 Raw detections: {len(detections)}")
                        # Apply accuracy improvements with very low threshold for visibility
                        detections = self.detection_system.filter_detections_by_confidence(detections, min_confidence=0.1)
                        print(f"🔍 After confidence filter: {len(detections)}")
                        detections = self.detection_system.apply_non_maximum_suppression(detections, iou_threshold=0.3)
                        print(f"🔍 After NMS filter: {len(detections)}")
                        
                        # Apply temporal tracking for consistency
                        detections = self.detection_system.track_detections_temporally(detections, self.previous_detections)
                        print(f"🔍 After temporal tracking: {len(detections)}")
                        
                        # Update previous detections for next frame
                        self.previous_detections = detections.copy()
                        
                        if detections:  # Only process if we still have detections after filtering
                            print(f"🔍 Processing {len(detections)} detections")
                            self.root.after(0, self._process_detection_results, detections)
                        else:
                            print("🔍 No detections after filtering")
                    else:
                        print("🔍 No raw detections found in this frame")
                    
                    # Check if uniform is complete
                    if hasattr(self, 'uniform_complete') and self.uniform_complete:
                        print("🛑 Stopping detection - Complete uniform detected")
                        break
                    
                    # Optimized sleep for better FPS
                    time.sleep(0.01)  # 10ms sleep for better balance
            ...
            try:
                if self.detection_system:
                    ...
                    self.detection_system = None
                print("🛑 Detection loop ended - resources cleaned up")
            except Exception as e:
                print(f"WARNING: Error during detection cleanup: {e}")
    ...
    def _update_camera_preview(self, frame):
        """Update camera preview with live detection overlays"""
        try:
            if not hasattr(self, 'camera_label') or not self.camera_label or not self.root.winfo_exists():
                return
            ...
            # Resize frame for portrait display (taller than wide)
            display_frame = cv2.resize(frame, (400, 500))  # Portrait aspect ratio
            
            # Convert BGR to RGB for tkinter
            rgb_frame = cv2.cvtColor(display_frame, cv2.COLOR_BGR2RGB)
            
            # Convert to PIL Image and PhotoImage for live feed
            try:
                from PIL import Image, ImageTk
                pil_image = Image.fromarray(rgb_frame)
                photo = ImageTk.PhotoImage(pil_image)
                
                # Update camera label with live frame (only if it exists and UI is ready)
                if hasattr(self, 'camera_label') and self.camera_label and self.root.winfo_exists():
                    self.camera_label.config(image=photo, text="")
                    self.camera_label.image = photo  # Keep a reference
                    print(f"SUCCESS: Live camera feed updated - FPS: {fps:.1f}, Resolution: {width}x{height}")
                else:
                    print("WARNING: Camera label not available for preview update")
                    
            except ImportError:
                # Fallback: Show detailed camera status with frame info
                if hasattr(self, 'camera_label') and self.camera_label and self.root.winfo_exists():
                    # Get detection status
                    detection_status = "INFO: AI Detection: ACTIVE"
                    if hasattr(self, 'detection_active') and self.detection_active:
                        detection_status = "INFO: AI Detection: RUNNING"
                    
                    # Get model info
                    model_info = getattr(self, 'current_model_path', 'Unknown')
                    
                    status_text = f"""🎥 LIVE CAMERA FEED
...
📹 Video feed captured successfully
🎯 Detection system operational

WARNING: Visual preview unavailable
(PIL ImageTk not installed)

💡 Camera detection works perfectly
even without visual preview!"""
                    ...
                    print("SUCCESS: Camera preview updated with detailed status")
                else:
                    print("WARNING: Camera label not available for preview update")
            
        except Exception as e:
            print(f"ERROR: Camera preview update error: {e}")
            # Show error status
            if hasattr(self, 'camera_label') and self.camera_label and self.root.winfo_exists():
                self.camera_label.config(
                    text=f"ERROR: CAMERA ERROR\n\n{str(e)}\n\nPlease check camera connection",
                    fg='#dc2626',
                    bg='#fee2e2',
                    font=('Arial', 10, 'bold')
                )
    
    def _process_detection_results(self, detections):
        """Process detection results with complete uniform detection logic"""
        try:
            current_time = time.time()
            ...
            for detection in detections:
                ...
                if confidence >= 0.50:
                    ...
                    # Track detected uniform components
                    self._track_uniform_component(class_name, confidence)
                    
                    # Add to activity log
                    log_message = f"Component Detected: {class_name} (Conf: {confidence:.2f})"
                    self.add_activity_log(log_message)
            
            # Check for complete uniform every 2 seconds
            if current_time - self.last_complete_check >= 2.0:
                self._check_complete_uniform()
                self.last_complete_check = current_time
                
        except Exception as e:
            print(f"ERROR: Error processing detection results: {e}")
    
    def _track_uniform_component(self, class_name, confidence):
        """Track detected uniform components"""
        try:
            # Get current person info
            if not hasattr(self, 'current_person_type') or not self.current_person_type:
                return
            
            person_type = self.current_person_type.lower()
            
            # Get student info for course and gender
            course = 'visitor'  # Default
            gender = 'male'     # Default
            
            if person_type == 'student' and hasattr(self, 'current_person_id'):
                student_info = self.get_student_info(self.current_person_id)
                if student_info:
                    course = student_info.get('course', 'tourism').lower()
                    gender = student_info.get('gender', 'male').lower()
            elif person_type == 'teacher':
                course = 'teacher'
            
            # Initialize tracking for this person if not exists
            person_key = f"{self.current_person_id}_{person_type}"
            if person_key not in self.detected_components:
                self.detected_components[person_key] = {
                    'course': course,
                    'gender': gender,
                    'components': {},
            ...
            self.detected_components[person_key]['components'][class_name] = {
                'confidence': confidence,
                'timestamp': time.time()
            }
            self.detected_components[person_key]['last_update'] = time.time()
            
            print(f"📋 Tracked component: {class_name} ({confidence:.2f}) for {course} {gender}")
            
        except Exception as e:
            print(f"ERROR: Error tracking uniform component: {e}")
    
    def _check_complete_uniform(self):
        """Check if all required uniform components are detected"""
        try:
            if not hasattr(self, 'current_person_id') or not self.current_person_id:
                return
            ...
            person_data = self.detected_components[person_key]
            course = person_data['course']
            gender = person_data['gender']
            detected_components = person_data['components']
            
            # Get required components for this course and gender
            if course not in self.required_components:
                course = 'tourism'  # Default fallback
            
            if gender not in self.required_components[course]:
                gender = 'male'  # Default fallback
            
            required = self.required_components[course][gender]
            detected = list(detected_components.keys())
            
            # Check if all required components are detected with confidence >= 0.50
            missing_components = []
            complete_components = []
            
            for component in required:
                if component in detected_components:
                    conf = detected_components[component]['confidence']
                    if conf >= 0.50:
                        complete_components.append(component)
                    else:
                        missing_components.append(f"{component} (low confidence: {conf:.2f})")
                else:
                    missing_components.append(component)
            
            # Determine status
            if len(complete_components) == len(required):
                status = "COMPLETE UNIFORM"
                self.compliant_count += 1
                self.uniform_complete = True
                log_message = f"SUCCESS: COMPLETE UNIFORM: All {len(required)} components detected"
                
                # Add detailed log
                self.add_activity_log(log_message)
                self.add_activity_log(f"Detected: {', '.join(complete_components)}")
                
                # Update displays
                self.root.after(0, self._update_detection_display, status, "uniform_check", 1.0)
                self.root.after(0, self._update_main_screen_with_detection_result, status, complete_components, missing_components)
                
                print(f"🔍 Uniform Check: {status} - {len(complete_components)}/{len(required)} components")
                
                # Process detection result for gate control
                detection_result = {
                    'compliant': True,
                    'violations': [],
                    'confidence': 1.0,
                ...
                self.root.after(0, self.process_detection_result_for_gate, detection_result)
                
                # Stop detection automatically when complete uniform is detected
                self.root.after(1000, self._auto_stop_detection_on_complete)
                
            else:
                status = "INCOMPLETE UNIFORM"
                self.violation_count += 1
                self.uniform_complete = False
                log_message = f"ERROR: INCOMPLETE UNIFORM: Missing {len(missing_components)} components"
                
                # Add detailed log
                self.add_activity_log(log_message)
                self.add_activity_log(f"Detected: {', '.join(complete_components)}")
                if missing_components:
                    self.add_activity_log(f"Missing: {', '.join(missing_components)}")
                
                # Update displays
                self.root.after(0, self._update_detection_display, status, "uniform_check", 1.0)
                self.root.after(0, self._update_main_screen_with_detection, status, "uniform_check", 1.0)
                
                # Process detection result for gate control
                detection_result = {
                    'compliant': False,
                    'violations': missing_components,
                    'confidence': len(complete_components) / len(required) if required else 0,
                    'complete_components': complete_components,
                    'missing_components': missing_components
                }
                self.root.after(0, self.process_detection_result_for_gate, detection_result)
            ...
            print(f"ERROR: Error checking complete uniform: {e}")
    
    def reset_uniform_tracking(self):
        """Reset uniform component tracking for new person"""
        self.detected_components = {}
        self.uniform_complete = False
        self.last_complete_check = 0
        print("INFO: Uniform tracking reset for new person")
    
    def _auto_stop_detection_on_complete(self):
        """Automatically stop detection when complete uniform is detected"""
        try:
            if self.uniform_complete and self.detection_active:
                # Log the automatic stop
                self.add_activity_log("🛑 Detection stopped automatically - Complete uniform detected")
                
                # Stop detection
                self.stop_detection()
                
                # Update main screen to show completion status
                if self.main_screen_window and self.main_screen_window.winfo_exists():
                    self._show_completion_status()
                
                print("SUCCESS: Detection stopped automatically - Complete uniform detected")
                
        except Exception as e:
            print(f"ERROR: Error in auto-stop detection: {e}")
    
    def _show_completion_status(self):
        """Show completion status on main screen"""
        try:
            ...
            completion_data = {
                ...
                'type': self.current_person_type,
                'action': 'ENTRY',
                'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                'status': 'COMPLETE UNIFORM',
                'confidence': 1.0,
                'class_name': 'uniform_complete'
            }
            
            # Add student info if available
            if self.current_person_type.lower() == 'student':
                student_info = self.get_student_info(self.current_person_id)
                if student_info:
                    completion_data.update(student_info)
            
            # Display completion status
            self.display_person_info(completion_data)
            
            # Show completion message for 3 seconds, then return to standby
            self.main_screen_window.after(3000, self.show_standby_message)
            
        except Exception as e:
            print(f"ERROR: Error showing completion status: {e}")
    
    def _update_main_screen_with_detection(self, status, class_name, confidence):
        """Update main screen with detection data"""
        try:
            if not self.current_person_id or not self.current_person_name:
                return
            ...
            # Update main screen
            self.update_main_screen_person(person_data)
            
            print(f"SUCCESS: Main screen updated with detection: {status} - {class_name} ({confidence:.2f})")
            
        except Exception as e:
            print(f"ERROR: Error updating main screen: {e}")
    
    def _update_detection_display(self, status, class_name, confidence):
        """Update detection results display in UI"""
        try:
            # Update detection statistics
            stats_text = f"Total: {self.total_detections} | Compliant: {self.compliant_count} | Violations: {self.violation_count}"
            
            # Log the detection result
            print(f"🎯 Detection Result: {status} - {class_name} ({confidence:.2f})")
            print(f"📊 Stats: {stats_text}")
            
            # Update activity log with statistics
            if self.total_detections % 5 == 0:  # Update every 5 detections
                self.add_activity_log(f"Detection Stats: {stats_text}")
                
        except Exception as e:
            print(f"ERROR: Error updating detection display: {e}")
    
    def simulate_detection_results(self):
        """Simulate detection results"""
        self.add_activity_log("Detection simulation started")
        self.add_activity_log("Simulated detection: COMPLIANT (Conf: 0.20)")
        self.add_activity_log("Detection simulation completed")
    
    def reset_camera_to_standby(self):
        """Reset camera label to standby mode with portrait orientation"""
        try:
            if hasattr(self, 'camera_label') and self.camera_label and self.root.winfo_exists():
                self.camera_label.config(
                    text="📷 CAMERA FEED (STANDBY MODE)\n\nCamera will activate when student\ntaps their ID for detection",
                    fg='#374151',
                    font=('Arial', 12),
                    bg='#dbeafe',
                    relief='sunken',
                ...
                print("WARNING: Camera label not available for reset")
        except Exception as e:
            print(f"WARNING: Error resetting camera to standby: {e}")
    
    def test_camera_availability(self):
        """Test if camera is available and working"""
        try:
            print("🔍 Testing camera availability...")
            self.add_activity_log("🔍 Testing camera availability...")
            ...
            for camera_index in [0, 1, 2]:
                try:
                    ...
                    if cap.isOpened():
                        ...
                        ret, frame = cap.read()
                        if ret and frame is not None:
                            print(f"✅ Camera found and working on index {camera_index}")
                            self.add_activity_log(f"✅ Camera found and working on index {camera_index}")
                            print(f"   Resolution: {frame.shape[1]}x{frame.shape[0]}")
                            self.add_activity_log(f"   Resolution: {frame.shape[1]}x{frame.shape[0]}")
                            cap.release()
            ...
            self.add_activity_log("❌ No working camera found")
            print("💡 Solutions:")
            self.add_activity_log("💡 Solutions:")
            print("   1. Connect a USB camera")
            self.add_activity_log("   1. Connect a USB camera")
            print("   2. Check camera permissions")
            self.add_activity_log("   2. Check camera permissions")
            print("   3. Make sure no other app is using the camera")
            self.add_activity_log("   3. Make sure no other app is using the camera")
            return False, None
